...
- Developing a Robust and Maintainable .NET Core Web API following Clean Architecture: API, Core, and Infrastructure layers as mentioned in this [Link](https://learn.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/common-web-application-architectures)

- The project shows CRUD operations on 2 entities **categories** and **products** having one to many relation.
...
- Unit testing covering the product service layer
...
## Clean Architecture
Clean Architecture is a software design pattern that aims to create systems that are easy to understand, maintain, and test by organizing code into distinct layers with clear responsibilities.
...
- **Core Layer**: The core layer contains the domain logic and business rules of the application. It encapsulates the essential functionality and represents the heart of the application's logic. This layer should be independent of any external frameworks or infrastructure concerns. It defines entities, data transfer objects, repository interfaces, services, and interfaces that model the business domain. The core layer should be highly testable (Unit testing) and contain the purest expression of the application's functionality.

- **Infrastructure Layer**: The infrastructure layer is responsible for providing implementations for external concerns such as databases, file systems, external services, logging, and caching. It contains concrete implementations of the repository interfaces defined in the core layer, enabling the application to interact with external resources.
...
## Repository Pattern
The Repository Design Pattern provides an abstraction layer between the application's business logic(Services) and the data access code (Entity framework in this repo), allowing for a separation of concerns and improved maintainability.
## Installation
...
Change the database connection string by your connection for the given key in **appsettings.json**
```
LocalHostDb
```
...
## API Reference for product entity
...
To run service unit tests for the products entity developed using XUnit
...
To run service integration test for the products controller developed using XUnit
